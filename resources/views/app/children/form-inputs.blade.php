...
<div class="row">
    ...
    <x-inputs.group class="col-sm-12">
        <x-inputs.text name="completename" label="Childs Complete Name"
            :value="old('completename', ($editing ? $child->completename : ''))" maxlength="50"
            placeholder="Child's Complete Name" required></x-inputs.text>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-6">
        <x-inputs.date name="dob" id="dob" onChange="calculateAge()" label="Date Of Birth"
            value="{{ old('dob', ($editing ? optional($child->dob)->format('Y-m-d') : '')) }}" max="255" required>
        </x-inputs.date>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-6">
        <x-inputs.text name="age" label="Age" max="255" required disabled></x-inputs.text>
    </x-inputs.group>
    ...
    <x-inputs.group class="col-sm-12 col-lg-3">
        <x-inputs.text name="phone" label="Phone" :value="old('phone', ($editing ? $child->phone : ''))" max="255"
            placeholder="Phone" required></x-inputs.text>
    </x-inputs.group>
    ...
    <x-inputs.group class="col-sm-12 col-lg-12">
        <x-inputs.text name="username" label="Username" :value="old('username', ($editing ? $child->username : ''))"
            maxlength="255" placeholder="Username" required autocomplete="off"></x-inputs.text>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-12">
        <x-inputs.password name="password" label="Password" :value="old('password', ($editing ? $child->password : ''))"
            maxlength="255" placeholder="password" required autocomplete="off"></x-inputs.password>
    </x-inputs.group>

    <x-inputs.group class="col-sm-12 col-lg-12">
        <x-inputs.password name="confirm-password" label="Confirm Password"
            :value="old('confirm-password', ($editing ? $child->confirm : ''))" maxlength="255"
            placeholder="Confirm Password" required></x-inputs.password>
    </x-inputs.group>
</div>
